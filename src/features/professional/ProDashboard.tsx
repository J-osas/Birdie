  ...
  return (
    <div className="w-full space-y-8">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[11px] font-bold uppercase tracking-[0.2em] text-[#660033]">Dashboard</p>
          <h1 className="text-3xl font-bold text-[#0A0A0A] mt-1">Welcome back, {firstName}</h1>
          <p className="text-[#615A5C] font-medium mt-1">
            {proProfile?.category || 'Professional'} · overview of jobs, earnings, and assessment
          </p>
        </div>
      ...
      {pending && (
        <div className="bg-amber-50 border border-amber-200 rounded-[1.75rem] p-6 text-sm text-amber-900 font-medium">
          Birdie ops is reviewing your documents and assessment. Payments, certifications, and profile edits unlock
          after verification.
        </div>
      )}
      ...
      <div className="grid lg:grid-cols-[1.1fr_0.9fr] gap-6 items-stretch">
        <div className="bg-[#660033] text-white rounded-[1.75rem] p-8 flex flex-col justify-between gap-6">
          <div>
            <p className="text-white/60 text-xs font-bold uppercase tracking-widest">Birdie assessment</p>
            ...
            <p className="text-sm text-white/75 font-medium mt-3 max-w-md">
              Your category skills score from onboarding. Clients can see this on your public profile.
            </p>
          </div>
          <Link
            to="/app/profile"
        ...
        <div className="bg-white border border-slate-200 rounded-[1.75rem] p-6 grid grid-cols-3 gap-4">
          ...
          <div>
            <p className="text-xs font-bold uppercase tracking-widest text-slate-400">Escrow</p>
      ...
      <div className="grid sm:grid-cols-3 gap-3">
        ...
        <div className="bg-white border border-slate-200 rounded-[1.75rem] p-5">
          ...
          <p className="text-lg font-bold text-[#0A0A0A] mt-2 capitalize">
            {(proProfile?.availability || Availability.UNAVAILABLE).replace(/_/g, ' ').toLowerCase()}
          </p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-[1.15fr_0.85fr] gap-6 items-start">
        <section className="space-y-4 min-w-0">
          <div className="flex justify-between items-center gap-2">
            <h2 className="text-xl font-bold text-[#0A0A0A]">Upcoming schedule</h2>
          ...
          {upcoming.length === 0 && (
            <div className="bg-white border border-dashed border-slate-200 rounded-[1.75rem] p-8 text-sm text-[#615A5C] font-medium">
              No upcoming dated jobs yet. Approved jobs with a start date show on Calendar.
            </div>
          )}

          <div className="flex justify-between items-center gap-2 pt-2">
            <h2 className="text-xl font-bold text-[#0A0A0A]">Recent jobs</h2>
          ...
          {jobs.length === 0 && (
            <div className="bg-white border border-dashed border-slate-200 rounded-[1.75rem] p-8 text-sm text-[#615A5C] font-medium">
              No assigned jobs yet. Once verified, matching clients can hire you from Find.
            </div>
          )}
        </section>
        ...
        <aside className="space-y-5">
          ...
          <div className="bg-white border border-slate-200 rounded-[1.75rem] p-6 space-y-4">
            <h2 className="font-bold text-[#0A0A0A]">Quick links</h2>
            <ul className="space-y-3 text-sm text-[#615A5C] font-medium">
              ...
              <li className="flex gap-3">
                ...
                <span>Stay verified to stay hireable</span>
              </li>
            </ul>
          </div>
          <div className="bg-white border border-slate-200 rounded-[1.75rem] p-6 space-y-4">
            <div className="flex justify-between items-center">
              <h2 className="font-bold text-[#0A0A0A]">Reviews</h2>
              <Link to="/app/reviews" className="text-[#660033] font-bold text-sm">
                View all
              </Link>
            </div>
            <p className="text-sm text-slate-500">
              {proProfile?.rating?.toFixed(1) || '0.0'} average · {reviews.length} total
            </p>
            ...
            {reviews.length === 0 && (
              <p className="text-slate-400 italic text-sm">Reviews appear after completed jobs.</p>
            )}
          </div>
        </aside>
      </div>
